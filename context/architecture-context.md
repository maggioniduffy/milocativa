...
- **Database (Supabase Postgres)**: all structured data — `people`, `items` (+ its subtype tables `estates`, `machinery`, `services`, `parking_spots`), `categories`, `availability`, `listings`, `rentals`, `payments`, `reviews`, `conversations`, `messages`, `notifications`, `push_subscriptions`. This is the single source of truth; there is no separate metadata/artifact split.
- **`items` uses class-table inheritance** (since migration `0011`): `items` holds only the columns common to every rentable thing (`id`, `item_kind`, `title`, `description`, `base_price`, `price_unit`, `status`, `category_id`, timestamps), and each subtype has its own 1:1 table keyed by `item_id` (PK + FK → `items.id`, `ON DELETE CASCADE`): `estates` (`area`, `bedrooms`, `address`, `location`, `condition`, `latitude`, `longitude`, `building_id`), `machinery` (`brand`, `model`, `year`, `location`, `condition`), `services` (`service_type`, `duration`), `parking_spots` (`spot_number`, `location`, `condition`, `building_id`). `item_kind` stays on `items` as the discriminator that says which subtype table to join. Every item row must have exactly one matching subtype row — created together at insert time. FKs from `listings`/`availability`/`rental_items` still point at `items.id`, unchanged. The subtype tables carry the same RLS pattern as `items`: public SELECT only when the item has a published listing, all writes admin-only.
